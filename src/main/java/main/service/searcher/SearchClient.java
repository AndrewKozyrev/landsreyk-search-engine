...
public class SearchClient {
  ...
  public SearchClient(SiteRepository siteRepository, PageRepository pageRepository,
      WordRepository wordRepository, JdbcTemplate jdbcTemplate) {
  ...
  private List<Word> mapToWords(Set<String> lemmas, Site site) {
    ...
    for (String lemma : lemmas) {
      ...
      if (pagesCount < 3 || percent < MAX_OCCURANCE_PERCENT) {
        words.add(word);
      }
    }
  ...
  public List<MatchedPage> search(String searchQuery, Site site) {
    ...
    try {
      ...
      lemmas = analyzer.getStats(searchQuery).keySet();
    } catch (IOException e) {
      LogUtil.logger.fatal(e);
    }
